 ...
 */
public class IRApplicationSession implements UpdateCallbackInterface {
  private static final String LOGTAG = "SampleAppSession";

  private WeakReference<Activity> mActivityRef;
  private final WeakReference<IRApplicationControl> mSessionControlRef;
  ...
  public IRApplicationSession(IRApplicationControl sessionControl) {
    mSessionControlRef = new WeakReference<>(sessionControl);
  }

  public IRApplicationSession(IRApplicationControl sessionControl, int videoMode) {
    mSessionControlRef = new WeakReference<>(sessionControl);
  ...
  public void initAR(Activity activity, int screenOrientation) {
    ...
    if (mInitVuforiaTask != null) {
      ...
      vuforiaException =
          new IRApplicationException(IRApplicationException.VUFORIA_ALREADY_INITIALIZATED,
              logMessage);
    ...
    if (vuforiaException == null) {
      try {
        ...
        vuforiaException =
            new IRApplicationException(IRApplicationException.INITIALIZATION_FAILURE, logMessage);
        Log.e(LOGTAG, logMessage);
      }
    }
  ...
  public boolean setFusionProviderType(int providerType) {
    ...
    if ((provider & ~providerType) != 0) {
      if (Vuforia.setAllowedFusionProviders(providerType)
          == FUSION_PROVIDER_TYPE.FUSION_PROVIDER_INVALID_OPERATION) {
  ...
  public void startAR(int camera) {
    ...
    try {
      ...
      vuforiaException =
          new IRApplicationException(IRApplicationException.CAMERA_INITIALIZATION_FAILURE,
              logMessage);
      Log.e(LOGTAG, logMessage);
    }
  ...
  public void stopAR() throws IRApplicationException {
    ...
    synchronized (mLifecycleLock) {
      ...
      if (!unloadTrackersResult) {
        throw new IRApplicationException(IRApplicationException.UNLOADING_TRACKERS_FAILURE,
            "Failed to unload trackers\' data");
      }

      if (!deinitTrackersResult) {
        throw new IRApplicationException(IRApplicationException.TRACKERS_DEINITIALIZATION_FAILURE,
  ...
  private void startCameraAndTrackers(int camera) throws IRApplicationException {
    String error;
    if (mCameraRunning) {
      error = "Camera already running, unable to open again";
      Log.e(LOGTAG, error);
      throw new IRApplicationException(IRApplicationException.CAMERA_INITIALIZATION_FAILURE, error);
    }
  ...
  private static class InitVuforiaTask extends AsyncTask<Void, Integer, Boolean> {
    ...
    private final WeakReference<IRApplicationSession> appSessionRef;

    InitVuforiaTask(IRApplicationSession session) {
      appSessionRef = new WeakReference<>(session);
    }

    protected Boolean doInBackground(Void... params) {
      IRApplicationSession session = appSessionRef.get();
    ...
    protected void onPostExecute(Boolean result) {
      ...
      if (result) {
        try {
          ...
          vuforiaException =
              new IRApplicationException(IRApplicationException.TRACKERS_INITIALIZATION_FAILURE,
                  logMessage);
          Log.e(LOGTAG, logMessage);
        }
      } else {
  ...
  private static class ResumeVuforiaTask extends AsyncTask<Void, Void, Void> {
    ...
    ResumeVuforiaTask(IRApplicationSession session) {
      appSessionRef = new WeakReference<>(session);
    }
  ...
  private static class InitTrackerTask extends AsyncTask<Void, Integer, Boolean> {
    ...
    InitTrackerTask(IRApplicationSession session) {
      appSessionRef = new WeakReference<>(session);
    }
    ...
    protected void onPostExecute(Boolean result) {
      ...
      if (result) {
        try {
          ...
          vuforiaException =
              new IRApplicationException(IRApplicationException.LOADING_TRACKERS_FAILURE,
                  logMessage);
        }
      } else {
  ...
  private static class LoadTrackerTask extends AsyncTask<Void, Void, Boolean> {
    ...
    LoadTrackerTask(IRApplicationSession session) {
      appSessionRef = new WeakReference<>(session);
    }
    ...
    protected void onPostExecute(Boolean result) {
      ...
      if (!result) {
        ...
        vuforiaException =
            new IRApplicationException(IRApplicationException.LOADING_TRACKERS_FAILURE, logMessage);
      } else {
  ...
  private static class StartVuforiaTask extends AsyncTask<Void, Void, Boolean> {
    ...
    StartVuforiaTask(IRApplicationSession session) {
      appSessionRef = new WeakReference<>(session);
    }
    ...
    protected Boolean doInBackground(Void... params) {
      ...
      synchronized (session.mLifecycleLock) {
        ...
        } catch (IRApplicationException e) {
    ...
    protected void onPostExecute(Boolean result) {
      ...
      IRApplicationControl sessionControl = appSessionRef.get().mSessionControlRef.get();
