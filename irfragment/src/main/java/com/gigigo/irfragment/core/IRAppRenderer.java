...
import com.gigigo.irfragment.utils.VideoBackgroundShader;
...
/**
 * The IRAppRenderer class handles the initialization and configuration of the
 ...
 */
public class IRAppRenderer {
  private static final String LOGTAG = "IRAppRenderer";

  private RenderingPrimitives mRenderingPrimitives = null;
  private IRAppRendererControl mRenderingInterface = null;
  ...
  public IRAppRenderer(IRAppRendererControl renderingInterface, Activity activity, int deviceMode,
      boolean stereo, float nearPlane, float farPlane) {
    this(renderingInterface, activity, deviceMode, CameraDevice.MODE.MODE_DEFAULT, stereo,
        nearPlane, farPlane);
  }

  public IRAppRenderer(IRAppRendererControl renderingInterface, Activity activity, int deviceMode,
      int videoMode, boolean stereo, float nearPlane, float farPlane) {
    ...
    device.setViewerActive(
        stereo);  // Indicates if the app will be using a viewer, stereo mode and initializes the rendering primitives
  ...
  private void initRendering() {
    vbShaderProgramID = IRUtils.createProgramFromShaderSrc(VideoBackgroundShader.VB_VERTEX_SHADER,
  ...
  public void render() {
    ...
    if (Renderer.getInstance().getVideoBackgroundConfig().getReflection()
      ...
      GLES20.glFrontFace(
          GLES20.GL_CW);  // Front camera. NOTE: Front camera Vuforia support has been deprecated
    } else {
    ...
    for (int v = 0; v < viewList.getNumViews(); v++) {
      ...
      GLES20.glViewport(viewport.getData()[0], viewport.getData()[1], viewport.getData()[2],
          viewport.getData()[3]);

      // Set scissor
      GLES20.glScissor(viewport.getData()[0], viewport.getData()[1], viewport.getData()[2],
          viewport.getData()[3]);

      // Get projection matrix for the current view.
      Matrix34F projMatrix =
          mRenderingPrimitives.getProjectionMatrix(viewID, state.getCameraCalibration());

      // Create GL matrix setting up the near and far planes
      float rawProjectionMatrixGL[] =
          Tool.convertPerspectiveProjection2GLMatrix(projMatrix, mNearPlane, mFarPlane).getData();

      // Apply the appropriate eye adjustment to the raw projection matrix, and assign to the global variable
      float eyeAdjustmentGL[] =
          Tool.convert2GLMatrix(mRenderingPrimitives.getEyeDisplayAdjustmentMatrix(viewID))
              .getData();
      ...
      // Call renderFrame from the app renderer class which implements IRAppRendererControl
  ...
  public void renderVideoBackground(State state) {
    if (currentView == VIEW.VIEW_POSTPROCESS) return;
    ...
    float[] vbProjectionMatrix =
        Tool.convert2GLMatrix(mRenderingPrimitives.getVideoBackgroundProjectionMatrix(currentView))
            .getData();
    ...
    GLES20.glVertexAttribPointer(vbVertexHandle, 3, GLES20.GL_FLOAT, false, 0,
        vbMesh.getPositions().asFloatBuffer());
    GLES20.glVertexAttribPointer(vbTexCoordHandle, 2, GLES20.GL_FLOAT, false, 0,
        vbMesh.getUVs().asFloatBuffer());
    ...
    GLES20.glDrawElements(GLES20.GL_TRIANGLES, vbMesh.getNumTriangles() * 3,
        GLES20.GL_UNSIGNED_SHORT, vbMesh.getTriangles().asShortBuffer());

    // Finally, we disable the vertex arrays
    GLES20.glDisableVertexAttribArray(vbVertexHandle);
    GLES20.glDisableVertexAttribArray(vbTexCoordHandle);

    IRUtils.checkGLError("Rendering of the video background failed");
  }
  ...
  private void storeScreenDimensions() {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
      ...
      WindowManager windowManager =
          (WindowManager) mActivityRef.get().getSystemService(Context.WINDOW_SERVICE);
