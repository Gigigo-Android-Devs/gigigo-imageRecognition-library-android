 ...
 */
@SuppressWarnings({ "unused", "SuspiciousNameCombination" }) public class IRUtils {

  private static final String LOGTAG = "IRUtils";
  ...
  private static int initShader(int shaderType, String source) {
    ...
    if (shader != 0) {
      ...
      if (glStatusVar[0] == GLES20.GL_FALSE) {
        Log.e(LOGTAG,
            "Could NOT compile shader " + shaderType + " : " + GLES20.glGetShaderInfoLog(shader));
  ...
  public static void screenCoordToCameraCoord(int screenX, int screenY, int screenDX, int screenDY,
      int screenWidth, int screenHeight, int cameraWidth, int cameraHeight, int[] cameraX,
      int[] cameraY, int[] cameraDX, int[] cameraDY, int displayRotation, int cameraRotation) {
  ...
  public static float[] getOrthoMatrix(float nLeft, float nRight, float nBottom, float nTop,
